 ...
 */
 ...
 void shape2(){
    ...
    // Disable lighting effects for wireframe
    glDisable(GL_LIGHTING);

    // Set the line color
    glColor3f(1.0, 1.0, 1.0);  // White

    // Draw each triangular face as a line loop
    glBegin(GL_LINE_LOOP);
    for (int i = 0; i < 60; i++) {
        for (int j = 0; j < 3; j++) {
            int v = dodecTriangles[i][j];
            glVertex3d(dodecVertices[v][0], dodecVertices[v][1], dodecVertices[v][2]);
        }
    }
    glEnd();
...
void shape3() {
    ...
    // Translate and rotate both the cone and cylinder together
    glPushMatrix();
    //glTranslatef(0.0, 2.0, 0);  // Translate along z-axis
    glRotatef(-90.0, 1.0, 0.0, 0.0);  // Rotate 90 degrees downwards
    glColor3ub(255, 165, 0); // Set color to orange

    glutSolidCylinder(radius, height, 32, 8);

    glTranslatef(0.0, 0.0, height);  // Translate along z-axis to place cone on top
    glColor3ub(255, 165, 0); // Set color to orange
...
void sphere(){
    glColor3f(1.0, 1.0, 0.0);  // Yellow for spheres
    glutSolidSphere(1.0, 32, 32);  // First sphere at (0, 0, 0)
}

void bar(){
    glColor3f(1.0, 0.0, 1.0);  // Pink for cylinder
    glRotatef(90.0, 0.0, 1.0, 0.0);  // Rotate 90 degrees around the y-axis
    glutSolidCylinder(0.5, 8.0, 32, 8);  // Cylinder connecting spheres
}
...
void shape5(){
    ...
    glTranslatef(4.0,4.0,0.0);
    sphere();
    glRotatef(-90.0, 0.0, 0.0, 1.0); //rotate downwards
    bar(); //right bar
    glPopMatrix();
    ...
    glTranslatef(4.0,-4.0,0.0);
    sphere();
    glRotatef(-180.0, 0.0, 1.0, 0.0); // Rotate left
    bar(); //bottom bar
    glPopMatrix();
}
...
void shape6(){
    ...
    glPushMatrix();
    shape5();
    glTranslatef(0.0,0.0,8.0); //translate toward viewer
    shape5();
    glPopMatrix();
    
    glTranslatef(4.0,4.0,0.0); //( right hand side )
    glRotatef(-90,0.0,1.0,0.0); //translte 90 degrees towards me 
    bar();
    
    glTranslatef(0.0,-8.0,0.0); //below above bar  (right side)
    glRotatef(-90,0.0,1.0,0.0); //translte 90 degrees towards me 
    bar();
    
    glTranslatef(-8.0,0.0,0.0); //to the left of above bar  (below, left side)
    glRotatef(-90,0.0,1.0,0.0); //translte 90 degrees towards me 
    bar();
    
    glTranslatef(0.0,8.0,0.0); //above of above defined bar  (top, left side)
    glRotatef(-90,0.0,1.0,0.0); //translte 90 degrees towards me 
    bar();
